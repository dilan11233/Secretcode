// src/app/api/room/role/route.ts
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const state = await applyRoleSelection(roomCode, playerId, isManager, roomToken); // ✅
    ...
    if (roomCode) await logAuditEvent({ roomCode, playerId, action: "role", status: "denied", reason: error instanceof Error ? error.message : "Failed.", ip });
